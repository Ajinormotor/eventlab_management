...
const search = () => {
    router.get(route('admin.user.show', props.events.id), { search: form.search });
};


const handleApprove = (event) => {
  console.log("Event id:", event.id);
  router.put(route('admin.event.approved', { event: props.events.data.id }));
};
...
<div class="flex items-center justify-start gap-20
   ...
   <div class="flex gap-3 items-center justify-start w-full">
    ...
    <div v-if="params.search" class="">
        <Link
        :href="route('dashboard.admin.event', { ...params, page: null, search:null })"
...
flex-col absolute top-[50px] z-10">
    ...
    <div class="w-full">
         ...
         <table class="w-full rounded-t-[12px]">
           ...
           <td 
            class="md:px-2 text-start group-hover:text-white">
            <div v-if="event.progress === 'canceled'"
            ...
            <div v-else
                   @click="handleApprove(event)"
      >
              <i class="ri-thumb-up-line text-red-500 text-base  -rotate-z-90"></i>
            </div>
          </td>
